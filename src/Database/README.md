...
Use **`Vortex\Database\Relation::belongsTo()`**, **`Relation::hasMany()`**, **`Relation::hasOne()`**, **`Relation::belongsToMany()`**, **`Relation::morphTo()`**, **`Relation::morphMany()`**, and **`Relation::morphOne()`** to build the same arrays as the bullet shapes above (keeps keys aligned with your relation methods).
