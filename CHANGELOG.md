...
- **JSON body shape validation:** **`Vortex\Support\JsonShape::validate()`** for structural/type checks on decoded JSON objects; **`Request::bodyShapeResponse()`** returns **`422`** via **`validationFailed()`** when the body does not match. Not JSON Schema; supports optional fields (`?type`) and types **`string`**, **`int`**, **`float`**, **`bool`**, **`number`**, **`array`**, **`list`**, **`object`**.
